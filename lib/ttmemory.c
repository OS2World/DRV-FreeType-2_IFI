...
/* -----------------------------------------------------------------
  ...
  /*                                                              */
  static
  void*  ft2_realloc( void* block, long size )
  {
    void*  newblk;
    void*  cpaddr;
    long*  oldhead;
    ...
    oldhead = ((long*)block) - 1;
    oldsize = *oldhead;

    COPY( "ft2_realloc: oldsize = " ); CATI( oldsize );
    CAT( "  newsize = " ); CATI( size ); CAT( "\n" ); WRITE;

    if ( size <= oldsize )
      return block;

    newblk = ft2_malloc( size );
    if ( !newblk ) {
      COPY( "ft2_realloc: dest malloc failed !!\n" ); WRITE;
      return NULL;
    }

    cpaddr = MEM_Copy( newblk, block, oldsize );
    if ( !cpaddr ) {
      COPY( "ft2_realloc: memcpy from old block failed !!\n" ); WRITE;
      if ( ft2_free( newblk ) ) {
         COPY( "ft2_realloc: freeing garbage block failed!!\n" ); WRITE;
      }
      return NULL;
    }

    COPY( "ft2_realloc: memcpy returns " ); CATH( cpaddr );
    CAT( "\n" ); WRITE;
